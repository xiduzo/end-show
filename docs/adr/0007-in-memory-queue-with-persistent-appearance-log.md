...
- **Persist nothing, derive Exposure Cap from in-memory ring buffer.** Rejected because the Cap window (60 min) is longer than realistic uptime guarantees on show day. We need the Cap to survive a hot-reload or container restart, otherwise a Student could blow past their cap by re-queuing right after a deploy.
